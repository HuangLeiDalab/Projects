...
struct Cloth
{
		  ...
		  float bendKd)
	{
		...
		for (int i = 0; i < triangleCount; i++){
			...
			mBendPairs[edge3].add(triangles[i].y);
		}

		// get the shared edge vertices
		sharedEdgeVerticeIndex.resize(triangleCount);
		for (int i = 0; i < triangleCount; i++){
			// Edge1
			clothEdge edge1(triangles[i].x, triangles[i].y);
			sharedEdgeVerticeIndex[i].x = 
				mBendPairs[edge1].p1 == triangles[i].z ? mBendPairs[edge1].p2 : mBendPairs[edge1].p1;
			// Edge2
			clothEdge edge2(triangles[i].y, triangles[i].z);
			sharedEdgeVerticeIndex[i].y = 
				mBendPairs[edge2].p1 == triangles[i].z ? mBendPairs[edge2].p2 : mBendPairs[edge2].p1;
			// Edge3
			clothEdge edge3(triangles[i].z, triangles[i].x);
			sharedEdgeVerticeIndex[i].z = 
				mBendPairs[edge3].p1 == triangles[i].z ? mBendPairs[edge3].p2 : mBendPairs[edge3].p1;
		}
	...
	void copySharedEdgeVertexIndex (glm::ivec3* dst)
	{
		for (int i = 0; i < sharedEdgeVerticeIndex.size(); ++i){
			dst[i] = sharedEdgeVerticeIndex[i];
		}
	}
	...
	int vertexCount;
	glm::vec3* vertices;
	// two vertice index that stand towards the same edge, 
	// if not exists or less than two vertices, replace it with index -1.
	std::vector<glm::ivec3> sharedEdgeVerticeIndex;
};
